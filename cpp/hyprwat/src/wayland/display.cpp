...
void WaylandDisplay::dispatch() { wl_display_dispatch(m_display); }

void WaylandDisplay::dispatchPending() { wl_display_dispatch_pending(m_display); }

void WaylandDisplay::roundtrip() { wl_display_roundtrip(m_display); }

void WaylandDisplay::prepareRead() {
    while (wl_display_prepare_read(m_display) != 0) {
        wl_display_dispatch_pending(m_display);
    }
}

void WaylandDisplay::readEvents() { wl_display_read_events(m_display); }

void WaylandDisplay::flush() { wl_display_flush(m_display); }

void WaylandDisplay::registry_handler(
    ...
    if (strcmp(interface, wl_compositor_interface.name) == 0) {
        self->m_compositor = static_cast<wl_compositor*>(wl_registry_bind(registry, id, &wl_compositor_interface, 4));
...
void WaylandDisplay::registry_remover(void*, wl_registry*, uint32_t) {}
